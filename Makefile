...
ifndef CONTRAIL_REPO_MIRROR_SNAPSHOT
	$(eval CONTRAIL_REPO_MIRROR_SNAPSHOT := 07262017)
endif
	...
	sed -i "s#^.*\(ocata main\)#deb [arch=amd64] $(CONTRAIL_REPO_MIRROR_URL)\/xenial-updates-$(CONTRAIL_SKU)\/$(CONTRAIL_REPO_MIRROR_SNAPSHOT)\/ xenial-updates-$(CONTRAIL_SKU) main#" $(KOLLA_DIR)/docker/base/sources.list.ubuntu
